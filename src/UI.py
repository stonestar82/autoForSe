...
class UI():
	def __init__(self, asyncio=asyncio, processAuto=ProcessAuto, processLab=ProcessLab, processPM=ProcessPM, processMinAnalyzer=ProcessMinAlanyzer):
		self.asyncio = asyncio
		self.processAuto = processAuto
		self.processLab = processLab
		self.processPM = processPM
		self.processMinAnalyzer = processMinAnalyzer
		self.processWeb = None
		self.httpd = None
		...
		self.switchIcon = ["c_switch_blue.png", "Switch Blue.png", "Router Blue.png", "Router.png"]
		...
		self.topNoteBook.add(self.automationFrame, text=" Automation & Lab ")
  
		self.pmFrame = ttk.Frame(self.root)
		...
		##### ipv4
		self.ipv4Frame = ttk.Frame(self.automationFrame)
		...
		##### ipv6
		self.ipv6Frame = ttk.Frame(self.automationFrame)
		...
		##### topology
		self.topologyFrame = ttk.Frame(self.automationFrame)
		...
		self.leafHostLabel = ttk.Label(self.topologyFrame, text="Leaf - Host 생성 : ")
		self.leafHostLabel.grid(row=8, column=1, padx=(8, 8), pady=(8, 8))
		self.leafHost = IntVar()
		self.leafHost.set(1)
		self.leafHostCheckBox = ttk.Checkbutton(self.topologyFrame, width=self.defaultComboBoxW, variable=self.leafHost)
		self.leafHostCheckBox.grid(row=8, column=2, padx=(8, 8), pady=(8, 8))
  
  
		self.switchIconLabel = ttk.Label(self.topologyFrame, text="Switch Icon : ")
		self.switchIconLabel.grid(row=9, column=1)
		...
		self.switchIconComboBox.grid(row=9, column=2, padx=(8, 8), pady=(8, 8))
		...
		##### 백업
		self.initFrame = ttk.Frame(self.automationFrame)
		...
		##### 핑테스트
		self.pingTestFrame = ttk.Frame(self.automationFrame)
		...
		##### 간편 topology 생성
		self.simpleTopologyFrame = ttk.Frame(self.automationFrame)
		...
		self.simpleLeafHostLabel = ttk.Label(self.simpleTopologyFrame, text="Leaf - Host 생성 : ")
		self.simpleLeafHostLabel.grid(row=10, column=1, padx=(8, 8), pady=(8, 8))
		self.simpleLeafHost = IntVar()
		self.simpleLeafHost.set(1)
		self.simpleLeafHostCheckBox = ttk.Checkbutton(self.simpleTopologyFrame, width=self.defaultComboBoxW, variable=self.simpleLeafHost)
		self.simpleLeafHostCheckBox.grid(row=10, column=2, padx=(8, 8), pady=(8, 8))
  
		self.simpleSwitchIconLabel = ttk.Label(self.simpleTopologyFrame, text="Switch Icon : ")
		self.simpleSwitchIconLabel.grid(row=11, column=1)
		...
		self.simpleSwitchIconComboBox.grid(row=11, column=2, padx=(8, 8), pady=(8, 8))
		...
		##### 정기정검 tab S #####
		self.pmNotebook = ttk.Notebook(self.pmFrame, width=620, height=500, )
		self.pmNotebook.pack()
  
		self.reportFrame = ttk.Frame(self.pmFrame)
		self.pmNotebook.add(self.reportFrame, text=" 정기점검 리포트 ")
  
		##### show tech to report
		self.showTechButton = tk.Button(self.reportFrame, text="정기점검 리포트 작성", width=self.longButtonW, command=lambda: self.loop.create_task(self.showtechToReport()))
		self.showTechButton.grid(row=0, column=1, sticky=tk.W, padx=(8, 8), pady=(8, 8))
  
		##### show tech to analysis
		self.showTechButton = tk.Button(self.reportFrame, text="Show Tech 분석", width=self.longButtonW, command=lambda: self.loop.create_task(self.showtechToAnalyzer()))
		self.showTechButton.grid(row=1, column=1, sticky=tk.W, padx=(8, 8), pady=(8, 8))

		##### show tech to analysis
		self.showTechButton = tk.Button(self.reportFrame, text="Show Tech 요약", width=self.longButtonW, command=lambda: self.loop.create_task(self.showtechSummary()))
		self.showTechButton.grid(row=2, column=1, sticky=tk.W, padx=(8, 8), pady=(8, 8))
  
		##### show tech 가져오기
		# self.showTechButton = tk.Button(self.reportFrame, text="Show Tech 파일 가져오기", width=self.longButtonW, command=lambda: self.loop.create_task(self.getShowtechFileCall()))
		# self.showTechButton.grid(row=3, column=1, sticky=tk.W, padx=(8, 8), pady=(8, 8))
		
		##### show tech 보기
		self.showTechButton = tk.Button(self.reportFrame, text="Show Tech 보기", width=self.longButtonW, command=lambda: self.loop.create_task(self.viewShowtech4Web()))
		self.showTechButton.grid(row=4, column=1, sticky=tk.W, padx=(8, 8), pady=(8, 8))
	...
	async def createTopology(self):
		...
		self.processAuto.createTopology(topology=topology, cpu=cpu, ram=ram, version=version, ethernetCount=ethernetCount, net=net, icon=icon, configInclude=configInclude, leafHost=leafHost)
	...
	async def createSimpleTopology(self):
		...
		leafHost = self.simpleLeafHost.get()
		...
		self.processAuto.createSimpleTopology(topology=topology, spinePrefix=spinePrefix, leafPrefix=leafPrefix, spinesCount=spineCount, leafsCount=leafCount, cpu=cpu, ram=ram, version=version, ethernetCount=ethernetCount, net=net, icon=icon, leafHost=leafHost)
	...
	async def showtechToReport(self):
		result, msg = self.processPM.showtechToReport()
  
		if result:
			self.gridReset()
			self.treeview.insert('', END, text='', values=("PM 리포트", "리포트 작성이 완료되었습니다.", datetime.now().strftime('%y-%m-%d %H:%M:%S')))
		else:
			messagebox.showwarning(title="warning", message=msg)

	async def showtechToAnalyzer(self):
		result, msg = self.processMinAnalyzer.alalyzer("./pm", datetime.now().strftime("%Y%m%d"))
		if result:
			self.gridReset()
			self.treeview.insert('', END, text='', values=("Show Tech 분석", "Show Tech 분석이 완료되었습니다.", datetime.now().strftime('%y-%m-%d %H:%M:%S')))
		else:
			messagebox.showwarning(title="warning", message=msg)

	async def getCmdResult(self, cmd):
		return self.processAuto.getCmdResultCall(cmd)

	async def getShowtechFileCall(self):
		cmd = "bash ls -tr /mnt/flash/schedule/tech-support/ | tail -1"
		result = await self.getCmdResult(cmd=cmd)

		self.processPM.getShowTechFile(result)
		# print(result)


	def httpServerStart(self):
		self.httpd = HTTPServer(('', 80), ProcessWeb)
		self.httpd.serve_forever()
   
  
	async def showtechSummary(self):
		self.processPM.showtechSummary("./pm/report/20230214/arista_analysis_20230214.xlsx")
   
	async def viewShowtech4Web(self):
   
		if not self.processWeb:
			self.httpdThread = Thread(target=self.httpServerStart)
